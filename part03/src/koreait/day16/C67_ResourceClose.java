...
//workspace UTF-8 이면 텍스트파일 UTF-8형식만
// 			MS949면 텍스트파일 ANSI 형식만

public class C67_ResourceClose {
	//안현기 try-with-resources 
	public static void main(String[] args) {
		
	//	String filename = "E:\\iclass05\\자바테스트.txt";	//읽기: 존재하는파일
	//	String filename2 = "E:\\iclass05\\자바테스트3.txt"; //쓰기 : 존재하지 않는 파일
		
		String filename =fileDialogOpen();	//복사할 파일은 지정하세요
		String filename2=fileDialogSave();	//저장할 파일은 폴더를 찾아가서 파일명을 직접쓰세요
		
		File ifile = new File (filename);	//지정된 경로의 파일명으로 파일객체 생성.
		File ofile = new File (filename2);
	
		try (Scanner sc= new Scanner(ifile);PrintWriter pw =new PrintWriter(ofile)){//System.in : 표준입력
			while(sc.hasNext()) {
	...
	// Windows OS는 응용프로그램들이 window라는 틀 안에서 만들어지는데 자바에서는 그것은 프레임이라고 합니다.
			...
			public static String fileDialogSave() {
				JFrame f= new JFrame();	//파일대화상자 시작할 프레임 객체 생성
